...
int main(int argc, char *argv[]) {
  ...
  ret = avcodec_parameters_to_context(cdc_ctx,
  ...
  int frame_width;
  int frame_height;
  size_t frame_character_buffer_size =  7680 * 4320 * 24  ; // worst case, some idiot with a 7680 x 4320 character terminal
  ...
  while ((ret = av_read_frame(fmt_ctx, packet)) >= 0) {
		//if (ret < 0) {
		//	av_strerror(ret, error_buffer,sizeof(error_buffer));
		//	fprintf(stderr, "Could not read frame from packet: %s\n", error_buffer);
		//	return 1;
		//}
    ...
    if (packet->stream_index == video_stream_index) {
      ret = avcodec_send_packet(cdc_ctx, packet);
			...
			if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
					break;  // no more frames for this packet, move on
			} else if (ret < 0) {
					av_strerror(ret, error_buffer, sizeof(error_buffer));
        ...
        if (frame_height > terminal_height || frame_width > terminal_width) {
          ...
          struct SwsContext *sws_ctx = sws_getContext( frame_width, frame_height, frame->format,resized_width,resized_height, AV_PIX_FMT_RGB24, SWS_FAST_BILINEAR, NULL, NULL, NULL);
					if (sws_ctx == NULL) {
          ...
          *ptr = '\0';
          ...
          printf("\033[H"); // new frame
					if (first_frame_clear == 0) {
						printf("\e[1;1H\e[2J"); // fixes uncleared screen problems when the video is too wide
						first_frame_clear++;
					}
